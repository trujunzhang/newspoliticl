...
// Filter upvoters by user's id named upvoterId.
function fitlerUpvoters(parameters, terms) {

    if (terms.upvoterId) {
        //const containedQuery = {upvoters: {$elemMatch: {$eq: ObjectId(user._id)}}};
        //  parameters.selector.push({ upvoters : { $elemMatch: { address: invitation.invitedUserEmail } } })
        parameters.selector.upvoters = {$elemMatch: {$eq: terms.upvoterId}};
    }
